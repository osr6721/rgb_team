...
# 이미지 사본을 만들어 녹색 부분을 빨간색으로 변경
    ...
    red_image[mask > 0] = [0, 0, 100]  # 녹색 부분을 빨간색으로 변경
...
# 결과 출력
    ...
    mask = cv2.bitwise_or(mask1, mask2)  # 두 빨간색 범위 마스크를 합침

    # 이미지 사본을 만들어 빨간색 부분을 회색으로 변경
    gray_image = image.copy()
    gray_image[mask > 0] = [128, 128, 128]  # 빨간색 부분을 회색으로 변경
    ...
    # 이미지 사본을 만들어 파란색 부분을 회색으로 변경
    gray_image = image.copy()
    gray_image[mask > 0] = [128, 128, 128]  # 파란색 부분을 회색으로 변경
